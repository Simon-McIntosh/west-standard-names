...
"""
    ...
    # Create individual name pages
    for name in sorted(all_names):
        content = f"""# {name}

{{% set item = standard_name_detail('{name}') %}}
...
"""
        ...
        with open(names_dir / f"{name}.md", 'w', encoding='utf-8') as f:
            f.write(content)
    
    print(f"Created {len(categories)} category pages and {len(all_names)} individual name pages")
